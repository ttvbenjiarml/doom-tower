...
namespace Doom.Displays
{
    ...
    /// </summary>
    public class DoomMultiDisplay : ModTowerDisplay<Doom>
    {
        private readonly int[] t = null!; // The tiers used for a particular instance of DoomBaseDisplay

        /// <summary>
        /// All classes that derive from ModContent MUST have a zero argument constructor to work
        /// </summary>
        public DoomMultiDisplay()
        {
        }

        public DoomMultiDisplay(int[] tiers)
        {
            t = tiers;
        }
        ...
        public override string Name => nameof(DoomMultiDisplay) + "-" + t.Printed();
            ...
            for (var i = 0; i <= 2; i++)
            {
                for (var j = 0; j <= 2; j++)
                {
                    for (var k = 0; k <= 2; k++)
                    {
                        ...
                        if (tiers.IsValid())
                        {
                            yield return new DoomMultiDisplay(tiers);
                        }
                    }
                }
            }
...
# if DEBUG
            ...
            SetMeshTexture(node, nameof(DoomBaseDisplay));
